...
import module.panther.*;
import module.phyloxml.*;
import java.io.File;
...
//        PantherJsonToPhyloXml("PTHR11705");
    ...
    static void convertAllInDirectory()
    {
        ...
        if (directoryListing != null)
        {
            ...
            {
                // NOTE IT WILL BUG OUT IF THERE IS .DS_STORE, DELETE .DS_STORE or files such as .gitignore
                PantherJsonToPhyloXml(child.getName().replace(".json", ""));
            }
        }
    }

    // Converts a single panther Json to Phylo Xml file
    // change path if json files placed elsewhere
    // assumes input files are in resource directory
    // only input name with file extension
    static void PantherJsonToPhyloXml(String fileName)
    {
        // declarations for mapping panther json objects to java objects
        ObjectMapper objectMapper = new ObjectMapper();
        Panther panther = new Panther();
        try
        {
            // mapping panther json file to pantherj java object
        ...
        {
            // creating the root node and setting up transformation(panther -> phyloxml)
            Phyloxml phylo = createPhyloRoot(loadHashWithFamNames(), panther);
            // transformation panther object to phyloxml object, build phyloxml tree while level order traversal
...
//    Sets clade attributes equal to module.panther.Annotation attributes but only if they are needed so XML file looks correct.
//    So we only allocate memory for objects when we see the module.panther.Annotation node attributes are not empty.
